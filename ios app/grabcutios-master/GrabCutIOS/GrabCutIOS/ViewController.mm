...
//        [timer invalidate];
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            ...
            for (int i = 0; i < 3; i++) {
                ...
                while (_resultImageView.image == nil) {}
                [timer0 invalidate];
                ...
                [resArr addObject:_resultImageView.image];
            }
            ...
            dispatch_async(dispatch_get_main_queue(), ^(void){
                ...
                i = 1;
                timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(test) userInfo:nil repeats:true];
            });
        });
...
-(void)test {
    ...
    BOOL cond = !CGRectIsNull(_grabRect) && !CGRectIsEmpty(_grabRect);
...
//    NSString *filename = [NSString stringWithFormat:@"test%i.jpeg", i];
//    [self setImageToTarget:[UIImage imageNamed:filename]];
    [self setImageToTarget:resArr[i - 1]];
    
}

-(void) testUsingImage:(UIImage *)image {
    ...
    if (!CGRectIsNull(_grabRect) && !CGRectIsEmpty(_grabRect)) {
        NSLog(@"processing...s");
        //        [self doGrabcut];
...
-(IBAction)tapOnSave:(id)sender{
    ...
    UIAlertView* alert = [[UIAlertView alloc] initWithTitle:@"Saved!" message:@"Image was saved to photo library." delegate:nil cancelButtonTitle:nil otherButtonTitles:@"OK"  , nil];
    [alert show];
}
